...
def train_model(X_tr, y_tr_bin, epochs=30, batch_size=64):
    ...
    dataset = TensorDataset(
        ...
        torch.tensor(y_tr_bin, dtype=torch.float32)
    )
...
print("\n===== MLP CRP CLASSIFIER (WITH DATA SCALING) =====")
...
for k in range(1, N):
    ...
    for train_subset in itertools.combinations(train_files, k):
        ...
        model = train_model(X_tr, y_tr_bin)

        device = next(model.parameters()).device
        with torch.no_grad():
            Xv = torch.tensor(X_val, dtype=torch.float32).to(device)
            val_scores = torch.sigmoid(model(Xv)).cpu().numpy().flatten()
        ...
        for test_file in test_candidates:
            ...
            with torch.no_grad():
                ...
                test_scores = torch.sigmoid(model(Xt)).cpu().numpy().flatten()
...
# SAVE RESULT
pd.DataFrame(results).to_csv("mlp_lodo_dataset_results_with_pct.csv", index=False)

print("\nSaved: mlp_lodo_dataset_results_with_pct.csv")